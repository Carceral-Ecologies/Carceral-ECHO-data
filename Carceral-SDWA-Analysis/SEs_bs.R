...
se <- with(Hp, sapply(seq(length(beta)), function(i, r = u[i,] * v[i,] / (-2*d) , m=ncol(u)-D) sqrt(sum(r[1:m]))))
...
bs_arr <- array(unlist(bs_points), dim = c(dim(bs_points[[1]]), length(bs_points)))
dimnames(bs_arr)[[2]] <- colnames(fit$par)

for(i in colnames(fit$par)) {
  message("**********************")  
  message("Submodel:", i)
  print(data.frame(estimate=fit$par[,i], se=se[,i]) |> 
          transform(p = format.pval(pnorm(abs(estimate), sd=se, lower.tail=FALSE)*2)) |>
          transform(
    se = round(se, 5),
    boot_estimate = 2*fit$par[,i] - rowMeans(bs_arr[1:nb,i,]),
    boot_se = apply(bs_arr[1:nb,i,], 1, sd),
    boot_lo = 2*fit$par[,i] - apply(bs_arr[1:nb,i,], 1, quantile, .975),
    boot_hi = 2*fit$par[,i] - apply(bs_arr[1:nb,i,], 1, quantile, .025)
